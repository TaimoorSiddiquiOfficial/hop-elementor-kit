...
class Import {
    ...
    public function ajax_actions( Ajax $ajax ) {
        $ajax->register_ajax_action( 'get_hop_template_data', function( $data ) {
            ...
            if ( ! empty( $data['editor_post_id'] ) ) {
                ...
                if ( ! get_post( $editor_post_id ) ) {
                    throw new \Exception( __( 'Post not found', 'hop-elementor-kit' ) );
                }
                Plugin::$instance->db->switch_to_post( $editor_post_id );
            }

            if ( empty( $data['template_id'] ) ) {
                throw new \Exception( __( 'Template id missing', 'hop-elementor-kit' ) );
            }
