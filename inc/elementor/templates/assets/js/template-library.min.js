!function($, $ele, $i) {
    window.liteTranslated = function (e, t) {
        return elementorCommon.translate(e, null, t, hopEditor.i18n)
    };
    ...
    $app.Behaviors.InsertTemplate = Marionette.Behavior.extend({
        ui: {
            insertButton: ".hopTemplateLibrary_insert-button"
        },
        events: {
    ...
    $app.Views.EmptyTemplateCollection = Marionette.ItemView.extend({
        ...
        template: "#hop-liteTemplateLibrary_empty",
        ui: {
    ...
    $app.Views.Loading = Marionette.ItemView.extend({
        template: "#hop-liteTemplateLibrary_loading",
        id: "liteTemplateLibrary_loading"
    }), 
    $app.Views.Logo = Marionette.ItemView.extend({
        template: "#hop-liteTemplateLibrary_header-logo",
    ...
    $app.Views.BackButton = Marionette.ItemView.extend({
        template: "#hop-liteTemplateLibrary_header-back",
    ...
    $app.Views.Menu = Marionette.ItemView.extend({
        template: "#hop-TemplateLibrary_header-menu",
    ...
    $app.Views.ResponsiveMenu = Marionette.ItemView.extend({
        template: "#hop-TemplateLibrary_header-menu-responsive",
    ...
    $app.Views.Actions = Marionette.ItemView.extend({
        template: "#hop-TemplateLibrary_header-actions",
    ...
    $app.Views.InsertWrapper = Marionette.ItemView.extend({
        template: "#hop-TemplateLibrary_header-insert",
    ...
    $app.Views.Preview = Marionette.ItemView.extend({
        template: "#hop-TemplateLibrary_preview",
    ...
    $app.Views.TemplateCollection = Marionette.CompositeView.extend({
        template: "#hop_TemplateLibrary_templates",
    ...
    $app.Views.Template = Marionette.ItemView.extend({
        template: "#hop-TemplateLibrary_template",
    ...
    $app.Modal = elementorModules.common.views.modal.Layout.extend({
        ...
        getTemplateActionButton: function(e) {
            var t = e.isPro && !hopEditor.hasPro ? "pro-button" : "insert-button";
            return viewId = "#hop-TemplateLibrary_" + t, template = Marionette.TemplateCache.get(viewId), Marionette.Renderer.render(template)
        },
        showLogo: function(e) {
    ...
    $app.Manager = function() {
        ...
        function addButton() {
            ...
            }), $sl.find(".dlhop_templates_add_button").length || $sl.find(HOP_FIND_SELECTOR).before($hopPpenLibraryButton)
        }

        function initAddButton($el) {
            var $t = $el.find(HOP_FIND_SELECTOR);
            $t.length && !$el.find(".dlhop_templates_add_button").length && $t.before($hopPpenLibraryButton), $el.on("click.onAddElement", ".elementor-editor-section-settings .elementor-editor-element-add", addButton)
        }

        function onChanges(t, i) {
            i.addClass("elementor-active").siblings().removeClass("elementor-active");
            var a = $dlHopResponsiveMap[t] || $dlHopResponsiveMap.desktop;
        ...
        function onEditorLoaded() {
            var $el = window.elementor.$previewContents,
                ...
                $el.on("click.onAddTemplateButton", ".dlhop_templates_add_button", $this.showModal.bind($this)), this.channels.tabs.on("change:device", onChanges)
        }

        var o, s, d, c, m, $this = this;
        console.log( window );

        HOP_FIND_SELECTOR = ".elementor-add-new-section .elementor-add-section-drag-title", 
        $hopPpenLibraryButton = '<div class="elementor-add-section-area-button dlhop_templates_add_button"> <img src="'+hopEditor.templateLogo+'" /> </div>', 
        $dlHopResponsiveMap = {
            desktop: "100%",
            tab: "768px",
            mobile: "360px"
        ...
        this.init = function() {
            $this.setFilter("type", hopEditor.default_tab, !0), $ele.on("preview:loaded", onEditorLoaded.bind(this))
        }, 
        this.getTabs = function() {
            var e = this.getFilter("type");
            return tabs = JSON.parse(hopEditor.tab_style)
        ...
        this.requestLibraryData = function(e) {
            ...
            e.forceSync && (t.data.sync = !0), elementorCommon.ajax.addRequest("get_hop_library_data", t)
        }, 
        this.requestTemplateData = function(e, t) {
            ...
            t && jQuery.extend(!0, i, t), elementorCommon.ajax.addRequest("get_hop_template_data", i)
        }, 
...
}(jQuery, window.elementor, window.hop || {});
